...
def choose_libraries(plex):
    print("Here are all the MOVIE and TV libraries on your Plex server: \n")
    ...
    choices = input("\nPlease enter the number of the library you want to select (separate multiple numbers with spaces): ")
    print()
...
def get_list(tmdb_api_key):
    list_id = input("Please enter the ID of the list: ")
    if list_id.isdigit():
...
def main():
    ...
    if not txt_files:
        ...
        if txt_file is None:
            print("\nThe list ID is not valid, please try again with the correct ID")
            return
        txt_files = [txt_file]
    else:
    ...
    for library in libraries:
            ...
            for movie in movies:
                ...
                if movie_info in map(lambda x: (clean_string(x[0]), x[1]), collection_movies):
                    if collection_name not in movie_ids['collections']:
                        print(f"\"{movie.title} ({year})\" has been added to the \"{collection_name}\" collection")
        ...
        if not added_to_collection:
            print(f"No items found in \"{library.title}\" to be added to the collection")
